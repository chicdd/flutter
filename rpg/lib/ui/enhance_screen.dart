...
// 전체화면 라우트(메뉴/모바일). 게임 내 NPC 는 EnhanceView 를 FloatingWindow 로 띄운다.
class EnhanceScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: EnhanceView(profile: profile),
    );
  }
}
...
class EnhanceView extends StatefulWidget {
  final PlayerProfile profile;
  const EnhanceView({super.key, required this.profile});

  @override
  State<EnhanceView> createState() => _EnhanceViewState();
}

class _EnhanceViewState extends State<EnhanceView> {
  GearItem? _sel;
  String? _msg;
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, i) {
                              ...
                              return ListTile(
                                ...
                                leading: ItemIcon(
                                  glyph: g.weaponType != null
                                      ? glyphForWeapon(g.weaponType!)
                                      : glyphForKind(g.kind),
                                  rarity: g.rarity,
                                  size: 34,
                                ),
                                title: EnhancedItemName.gear(g, fontSize: 12),
                                onTap: () => setState(() {
                                  _sel = g;
                                  _msg = null;
                                }),
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    EnhancedItemName.gear(g, fontSize: 18, maxLines: 2),
                    Text(
                      '${g.rarity.label} · ${g.kind.baseName} · iLv ${g.itemLevel}',
          ...
          if (info.maxed)
            ...
            Row(
              children: [
                Expanded(
                  child: _prob('유지', info.keep, const Color(0xFF90A4AE)),
                ),
                const SizedBox(width: 6),
                Expanded(
                  child: _prob('하락', info.downgrade, const Color(0xFFFF8A65)),
                ),
                const SizedBox(width: 6),
                Expanded(
                  child: _prob('파괴', info.destroy, const Color(0xFFD32F2F)),
                ),
              ],
            ),
            const SizedBox(height: 12),
    ...
    return Container(
      ...
      child: Column(
        children: [
          Text(
            label,
            style: TextStyle(color: Colors.white70, fontSize: big ? 13 : 11),
          ),
  ...
  List<Widget> _statPreview(GearItem g) {
    ...
    void row(String label, double a, double b, {bool pct = false}) {
      if (a == 0 && b == 0) return;
      String f(double v) =>
          pct ? '${(v * 100).toStringAsFixed(1)}%' : v.toStringAsFixed(1);
      rows.add(
        Padding(
