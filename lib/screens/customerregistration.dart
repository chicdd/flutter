...
class CustomerRegistrationState extends State<CustomerRegistration> {
  ...
  final List<CodeData> _additionalServiceCodeList = [];
  final List<CodeData> _additionalProvisionCodeList = [];
  ...
  final _newDvrDateController = TextEditingController();
  final List<CodeData> _dvrTypeCodeList = [];
  ...
  bool _validateInputs() {
    if (_managementNumberController.text.isEmpty) {
      showToast(context, message: '관제관리번호를 입력해주세요.');
      return false;
    }

    if (_controlTypeController.text.isEmpty) {
      showToast(context, message: '관제상호명을 입력해주세요.');
      return false;
    }

    if (_addressController.text.isEmpty) {
      showToast(context, message: '물건지주소를 입력해주세요.');
      return false;
    }
  ...
  Future<void> saveCustomer() async {
    ...
    try {
      ...
      final success = await DatabaseService.insertCustomer(
        ...
        data: {
          // 기본 정보
          '관제관리번호': _managementNumberController.text,
          '영업관리번호': _erpCusNumberController.text,
          ...
          '소방서코드': _emergencyContactController.text,
          ...
          '미경계위치': _mainLocationController.text,
          ...
          'TMP1': _acquisitionController.text,
          '키박스번호': _keyBoxesController.text,
          'TMP2': _keypadController.text,
          'TMP3': _keypadQuantityController.text,
          'DVR여부': _isDvrInspection ? 1 : 0,
          'TMP8': _isWirelessSensorInspection ? 1 : 0,
          '관제액션': _controlActionController.text,
          '메모': _memo1Controller.text,
          '메모2': _memo2Controller.text,

          // 확장 정보
          'cu1': _openingPhoneController.text,
          'cu2': _modemSerialController.text,
          'cu3': _openingDateController.text,
          'cu4': _additionalMemoController.text,
          'tmP4': _gpsX1Controller.text,
          'tmP5': _gpsY1Controller.text,
          'tmP6': _gpsX2Controller.text,
          'tmP7': _gpsY2Controller.text,
  ...
  void _clearAllFields() {
    ...
    // 부가서비스
    _newAsDateController.clear();
    _newAsMemoController.clear();

    // DVR
    _newDvrAddressController.clear();
    _newDvrPortController.clear();
    _newDvrIdController.clear();
    _newDvrPasswordController.clear();
    _newDvrDateController.clear();
    ...
    setState(() {
      ...
      _newAsServiceCode = null;
      _newAsProvisionCode = null;
      _additionalServiceItems = [];
      _newDvrConnectionMethod = false;
      _newDvrTypeCode = null;
      _dvrItems = [];
                  ...
                  IntrinsicHeight(
                    child: Row(
                      children: [
                        ...
                        Expanded(child: _buildDetailInfoSection()),
                        const SizedBox(width: 16),
                  ...
                  IntrinsicHeight(
                    child: Row(
                      children: [
                        ...
                        const SizedBox(width: 16),
                        Expanded(child: _buildSecuritySettingsSection()),
                      ],
                    ),
                  ),
                  const SizedBox(height: 16),

                  // 6행: 가로 나열 (GPS | 회사/지사 | 전용회선)
                  IntrinsicHeight(
                    child: Row(
                      children: [
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Expanded(
                child: BuildDropdownField(
                  label: '부가서비스',
                  value: _newAsServiceCode,
                  items: _additionalServiceCodeList,
                  searchQuery: '',
                  onChanged: (v) => setState(() => _newAsServiceCode = v),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: BuildDropdownField(
                  label: '부가서비스제공',
                  value: _newAsProvisionCode,
                  items: _additionalProvisionCodeList,
                  searchQuery: '',
                  onChanged: (v) => setState(() => _newAsProvisionCode = v),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: CommonTextField(
                  label: '일자',
                  controller: _newAsDateController,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: CommonTextField(
                  label: '메모',
                  controller: _newAsMemoController,
                ),
              ),
              const SizedBox(width: 12),
              ElevatedButton(
                onPressed: () {
                  if (_newAsServiceCode == null) return;
                  setState(() {
                    _additionalServiceItems.add({
                      'serviceCode': _newAsServiceCode ?? '',
                      'provisionCode': _newAsProvisionCode ?? '',
                      'date': _newAsDateController.text,
                      'memo': _newAsMemoController.text,
                    });
                    _newAsServiceCode = null;
                    _newAsProvisionCode = null;
                    _newAsDateController.clear();
          ...
          if (_additionalServiceItems.isNotEmpty) ...[
            const SizedBox(height: 12),
            // 헤더
            Container(
            ...
            ...List.generate(_additionalServiceItems.length, (i) {
              final item = _additionalServiceItems[i];
              return Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 12,
                  vertical: 10,
                ...
                child: Row(
                  children: [
                    ...
                    InkWell(
                      onTap: () =>
                          setState(() => _additionalServiceItems.removeAt(i)),
                      child: const Icon(
                        Icons.close,
                        size: 16,
                        color: Colors.red,
        ...
        children: [
          ...
          Row(
            children: [
              BuildRadioOption(
                label: 'CS',
                value: !_newDvrConnectionMethod,
                onChanged: (_) =>
                    setState(() => _newDvrConnectionMethod = false),
              ),
              const SizedBox(width: 16),
              BuildRadioOption(
                label: 'WEB',
                value: _newDvrConnectionMethod,
                onChanged: (_) =>
                    setState(() => _newDvrConnectionMethod = true),
              ),
            ],
          ),
          const SizedBox(height: 16),
          Row(
            ...
            children: [
              Expanded(
                child: BuildDropdownField(
                  label: 'DVR종류',
                  value: _newDvrTypeCode,
                  items: _dvrTypeCodeList,
                  searchQuery: '',
                  onChanged: (v) => setState(() => _newDvrTypeCode = v),
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: CommonTextField(
                  label: '접속주소',
                  controller: _newDvrAddressController,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: CommonTextField(
                  label: '접속포트',
                  controller: _newDvrPortController,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: CommonTextField(
                  label: '접속 ID',
                  controller: _newDvrIdController,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: CommonTextField(
                  label: '접속 암호',
                  controller: _newDvrPasswordController,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: CommonTextField(
                  label: '추가일자',
                  controller: _newDvrDateController,
                ),
              ),
              const SizedBox(width: 12),
              ElevatedButton(
                onPressed: () {
                  setState(() {
                    _dvrItems.add({
                      'connectionMethod': _newDvrConnectionMethod,
                      'typeCode': _newDvrTypeCode ?? '',
                      'address': _newDvrAddressController.text,
                      'port': _newDvrPortController.text,
                      'id': _newDvrIdController.text,
                      'password': _newDvrPasswordController.text,
                      'date': _newDvrDateController.text,
                    });
                    _newDvrConnectionMethod = false;
                    _newDvrTypeCode = null;
                    _newDvrAddressController.clear();
          ...
          if (_dvrItems.isNotEmpty) ...[
            ...
            Container(
              ...
              child: Row(
                children: [
                  for (final h in ['접속방식', 'DVR종류', '접속주소', '포트', 'ID', '추가일자'])
                    Expanded(
                      child: Text(
                        h,
                        style: TextStyle(
            ...
            ...List.generate(_dvrItems.length, (i) {
              final item = _dvrItems[i];
              return Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 12,
                  vertical: 10,
                ...
                child: Row(
                  children: [
                    ...
                    InkWell(
                      onTap: () => setState(() => _dvrItems.removeAt(i)),
                      child: const Icon(
                        Icons.close,
                        size: 16,
                        color: Colors.red,
