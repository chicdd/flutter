...
class _CustomerSearchPanelState extends State<CustomerSearchPanel> {
  ...
  Future<void> _loadCustomers() async {
    ...
    try {
      final loadedCustomers = await DatabaseService.getCustomers(count: 100);
      setState(() {
  ...
  void _onSearchChanged(String query) {
    ...
    _debounceTimer = Timer(const Duration(milliseconds: 500), () {
      if (mounted) {
        ...
        // 다른 필터는 3글자 이상일 때만 서버 API 호출
        if (selectedFilter == '사용자HP' && query.length >= 1) {
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _getStatusColor(
                          customer.controlCustomerStatusName ?? '정상',
                        ).withOpacity(0.1),
                        borderRadius: BorderRadius.circular(4),
                        border: Border.all(
                          color: _getStatusColor(
