...
class DatabaseService {
  ...
  static Future<List<Customer>> getCustomers({int count = 100}) async {
    try {
      ...
      final request = await httpClient.getUrl(
        Uri.parse('https://localhost:7088/api/관제고객/top?count=100'),
      );
      print(Uri);
  ...
  static Future<List<Customer>> searchCustomersLocal({
    required String filterType,
    required String query,
    int count = 100,
  }) async {
    try {
      ...
      final customers = await getCustomers(count: count);
